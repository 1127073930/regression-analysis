...
# -------------------------------
# 数据文件位于当前工作目录下
data_raw <- read_csv("galtonfamiliesall.csv")
...
# -------------------------------
# 计算相关系数
cor_xy <- cor(data_processed$MidParent, data_processed$Height)
cat("父母平均身高与子女身高的相关系数：", round(cor_xy, 4), "\n")

# 绘制散点图 + 回归线
scatter_plot <- ggplot(data_processed, aes(x = MidParent, y = Height)) +
...
# 4. 拟合一元线性回归模型
# -------------------------------
model <- lm(Height ~ MidParent, data = data_processed)
...
# 多元回归
multi_model <- lm(Height ~ Father + Mother + Child, data = data_processed)
summary(multi_model)
